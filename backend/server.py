...
try:
    ...
    from routes import (
        ...
        sso_router, wizard_router, strategies_router, channels_router, avatar_router,
    )
    # region agent log
...
async def check_maintenance_mode(request: Request, call_next):
    # Bypass health checks and admin paths
    if request.url.path.startswith("/api/admin") or request.url.path in ["/api/health", "/api/health/db"]:
        return await call_next(request)
    
    try:
        from config import db as _db
        if _db is not None:
            settings = _db.system_settings.find_one({"key": "global"}) or {}
            if settings.get("maintenance_mode", False):
                return JSONResponse(
                    status_code=503,
                    content={"detail": "System is undergoing maintenance. Please try again later."},
...
try:
    from seed_strategies import seed_curated_strategies
    seed_curated_strategies()
except Exception as _seed_err:
    import logging
    logging.warning(f"Curated strategy seed skipped: {_seed_err}")
