...
try:
    from routes.sso import ALLOWED_CLIENTS
except ModuleNotFoundError:
    try:
        from backend.routes.sso import ALLOWED_CLIENTS
    except ModuleNotFoundError:
        from .sso import ALLOWED_CLIENTS
...
@router.post("/api/crm/ingest")
async def crm_partner_ingest(
    data: IngestPayload,
    x_partner_client_id: Optional[str] = Header(None, alias="X-Partner-Client-ID"),
    x_partner_client_secret: Optional[str] = Header(None, alias="X-Partner-Client-Secret")
):
    if db is None:
        raise HTTPException(status_code=503, detail="Database unavailable")

    # 1. Authenticate partner headers
    if not x_partner_client_id or not x_partner_client_secret:
        raise HTTPException(status_code=401, detail="Authentication credentials missing")

    client = ALLOWED_CLIENTS.get(x_partner_client_id)
    if not client or client.get("client_secret") != x_partner_client_secret:
        raise HTTPException(status_code=401, detail="Invalid partner credentials")

    if data.partner_id != x_partner_client_id:
        raise HTTPException(status_code=400, detail="Partner ID mismatch")

    # 2. Resolve default owner_id (admin or first user)
    admin_user = db.users.find_one({"$or": [{"role": "admin"}, {"is_admin": True}]})
    if not admin_user:
        admin_user = db.users.find_one({})  # fallback
    owner_id = admin_user["_id"] if admin_user else None

    # 3. Handle client matching & contact resolution
    email_lower = data.client_email.lower()
    ...
    if contact:
        ...
        update_fields = {}
        if not contact.get("phone") and data.client_phone:
            update_fields["phone"] = data.client_phone
        
        # Merge names if name is empty in DB
        if not contact.get("first_name") and data.client_name:
            parts = data.client_name.strip().split(" ", 1)
            update_fields["first_name"] = parts[0]
            if len(parts) > 1 and not contact.get("last_name"):
                update_fields["last_name"] = parts[1]

        # Update tags to include partner sync reference
        tags = contact.get("tags") or []
        sync_tag = f"partner_{data.partner_id}"
        if "partner_sync" not in tags:
            tags.append("partner_sync")
        if sync_tag not in tags:
            tags.append(sync_tag)
        update_fields["tags"] = tags
        update_fields["updated_at"] = datetime.now(timezone.utc)

        db.contacts.update_one({"_id": contact_id}, {"$set": update_fields})
    else:
        ...
        new_contact = {
            ...
            "company": "",
            "job_title": "",
            "tags": ["partner_sync", f"partner_{data.partner_id}"],
            "source": "partner_site",
            "notes": [],
            "engagement": {
                "emails_sent": 0, "emails_opened": 0, "emails_clicked": 0,
                "whatsapp_messages": 0, "sms_messages": 0
            },
            "status": "active",
            "created_at": datetime.now(timezone.utc),
            "updated_at": datetime.now(timezone.utc),
        }
        res = db.contacts.insert_one(new_contact)
        contact_id = res.inserted_id

    # 4. Insert dynamic lead event in crm_leads collection
    lead_record = {
    ...
    db.crm_leads.insert_one(lead_record)

    return {
        "success": True,
        "contact_id": str(contact_id),
        "message": f"Successfully ingested event '{data.event_type}' for {email_lower}"
    }
