...
async def create_support_ticket(data: SupportTicketCreate):
    ...
    # CRM Synchronization (Contact & Lead Ingestion)
    if data.email:
        try:
            email_lower = data.email.lower()
            contact = db.contacts.find_one({"email": email_lower})
            
            # Resolve owner (admin or first user)
            ...
            if contact:
                ...
                update_fields = {}
                if not contact.get("phone") and data.phone:
                ...
                if "support_ticket" not in tags:
                    tags.append("support_ticket")
                ...
                if data.client_name:
                    ...
                    try:
                        user_record = db.users.find_one({"_id": ObjectId(data.user_id)})
                        if user_record:
            ...
            # Insert a lead event in crm_leads
            lead_record = {
                "contact_id": contact_id,
                "partner_id": "intermaven_support",
            ...
            # Prevent CRM sync failures from blocking support ticket logging
            print(f"CRM sync error in create_support_ticket: {crm_err}")
...
async def list_forum_posts(category: Optional[str] = None):
    query = {"status": "published"}
    if category:
        query["category"] = category
    posts = list(db.forum_posts.find(query).sort("created_at", -1).limit(50))
    for post in posts:
        post["_id"] = str(post["_id"])
        if isinstance(post.get("created_at"), datetime):
            post["created_at"] = post["created_at"].isoformat()
        if "slug" not in post:
            post["slug"] = f"ticket-{post.get('ticket_id')}"
    return {"posts": posts}


@router.get("/api/forum/posts/{slug}")
async def get_forum_post(slug: str):
    query = {"slug": slug, "status": "published"}
    if slug.startswith("ticket-"):
        ticket_id = slug.replace("ticket-", "")
        query = {"ticket_id": ticket_id, "status": "published"}
    
    post = db.forum_posts.find_one(query)
    if not post:
