...
function localizeAuthUI() {
  ...
  var reg = window.IntermavenRegion ? window.IntermavenRegion.getRegion() : { country: 'KE', currency: 'KES' };
  
  var label = document.querySelector('#ws1 label[for="rph"]') || document.querySelector('#ws1 label:nth-of-type(3)');
  var phoneInput = document.getElementById('rph');
  var phoneHint = document.querySelector('#ws1 .fh');
  var subtitle = document.querySelector('#ws1 .wsu');
  
  if (isWestern) {
    if (label) label.textContent = 'Phone (WhatsApp)';
    if (phoneInput) phoneInput.placeholder = '+1 (555) 000-0000';
    if (phoneHint) phoneHint.textContent = 'Used for WhatsApp notifications';
    if (subtitle) subtitle.textContent = 'Join independent creators building with Intermaven.';
  } else {
    if (label) label.textContent = 'Phone (M-Pesa & WhatsApp)';
    if (phoneInput) phoneInput.placeholder = '+254 7XX XXX XXX';
    if (phoneHint) phoneHint.textContent = 'Used for M-Pesa payments and WhatsApp notifications';
    if (subtitle) subtitle.textContent = 'Join African creatives building with Intermaven.';
  }
  
  var prices = {
    free: isWestern ? '$0' : 'KES 0',
    creator: isWestern ? '$5' : 'KES 500',
    pro: isWestern ? '$15' : 'KES 1,500'
  };
  
  var pFree = document.querySelector('#pl-free .ppr');
  var pCreator = document.querySelector('#pl-creator .ppr');
  var pPro = document.querySelector('#pl-pro .ppr');
  
  if (pFree) pFree.textContent = prices.free;
  if (pCreator) pCreator.textContent = prices.creator;
  if (pPro) pPro.textContent = prices.pro;
  
  PDET.free = isWestern ? '150 free credits \u2022 All AI tools \u2022 Social AI (2 accounts)' : '150 free credits \u2022 All AI tools \u2022 Social AI (2 accounts)';
  PDET.creator = isWestern ? '600 credits \u2022 Priority speed \u2022 Social AI (6 accounts) \u2022 Save outputs' : '600 credits \u2022 Priority speed \u2022 Social AI (6 accounts) \u2022 Save outputs';
  PDET.pro = isWestern ? '2500 credits \u2022 White-label \u2022 Unlimited accounts \u2022 API access' : '2500 credits \u2022 White-label \u2022 Unlimited accounts \u2022 API access';

  var pmMpesa = document.getElementById('pm-mpesa');
  var secMpesa = document.getElementById('sec-mpesa');
  
  if (isWestern) {
    var isUSorCA = ['US', 'CA'].includes(reg.country.toUpperCase());
    if (isUSorCA) {
      if (pmMpesa) {
        pmMpesa.innerHTML = '<span class="pmi">&#128242;</span>Venmo';
        pmMpesa.style.display = 'block';
      }
      if (secMpesa) {
        secMpesa.innerHTML = '<div class="mbx"><p style="font-size:10px;color:var(--mu);margin-bottom:4px">Venmo Username</p><div class="mco">@intermaven</div><p style="font-size:11px;color:var(--mu);margin-top:3px">Include your email in transaction notes.</p></div><div class="fi"><label>Venmo Transaction ID</label><input id="mcod" placeholder="e.g. 1234567890" style="text-transform:uppercase"/></div>';
      }
    } else {
      if (pmMpesa) {
        pmMpesa.innerHTML = '<span class="pmi">&#128187;</span>Apple Pay';
        pmMpesa.style.display = 'block';
      }
      if (secMpesa) {
        secMpesa.innerHTML = '<div style="text-align:center;padding:16px;background:var(--bg3);border:1px solid var(--b1);border-radius:var(--r);margin-bottom:11px"><div style="font-size:26px;margin-bottom:5px">&#127822;</div><p style="font-size:11px;color:var(--mu)">Apple Pay enabled. Press pay to continue.</p></div>';
      }
    }
  } else {
    if (pmMpesa) {
      pmMpesa.innerHTML = '<span class="pmi">&#128242;</span>M-Pesa';
      pmMpesa.style.display = 'block';
    }
    if (secMpesa) {
      secMpesa.innerHTML = '<div class="mbx"><p style="font-size:10px;color:var(--mu);margin-bottom:4px">Paybill number</p><div class="mco">522900</div><p style="font-size:11px;color:var(--mu);margin-top:3px">Account: <strong style="color:var(--tx)">IVM-00001</strong></p></div><div class="fi"><label>M-Pesa confirmation code</label><input id="mcod" placeholder="e.g. QGH7X2ABC1" style="text-transform:uppercase"/></div>';
    }
  }
}
...
function finishOnboard() {
  ...
  var isWestern = window.IntermavenRegion && window.IntermavenRegion.isWesternRegion();
  var fallbackPh = isWestern ? '+1 (555) 000-0000' : '+254 7XX XXX XXX';
  setTimeout(function() { toast('WhatsApp message sent', '', '&#128172;', 'Welcome sent to ' + (S.ph || fallbackPh)); }, 2500);
}
