...
function HomePage({ portal = 'music', subdomainPage = null, onOpenAppModal, onOpenAuth, onToast }) {
  ...
  const localizeSlides = (slidesList) => {
    if (!isWestern || !slidesList) return slidesList;
  ...
  let localizedPortalData = { ...portalData };
  if (isWestern) {
    localizedPortalData.slides = localizeSlides(portalData.slides);
    if (portalData.ftitle) {
      localizedPortalData.ftitle = portalData.ftitle.replace('Nairobi', 'Atlanta').replace('African', 'independent');
    }
    if (portalData.btitle) {
      localizedPortalData.btitle = portalData.btitle.replace('Nairobi', 'Atlanta').replace('African', 'independent');
    }
    if (portalData.bsub) {
      localizedPortalData.bsub = portalData.bsub.replace('Nairobi', 'Atlanta').replace('African', 'independent');
    }
  }

  const pageData = subdomainPage && SUBDOMAIN_PAGES[subdomainPage]
    ? { ...localizedPortalData, ...SUBDOMAIN_PAGES[subdomainPage] }
    : localizedPortalData;
  const slides = pageData.slides || localizedPortalData.slides;
        ...
        <div className="hs">
          <div className="hcont">
            ...
            <div className={`hb hbtns ${slideState}`}>
              <Link 
                to={currentSlideData.b1link} 
                state={currentSlideData.b1link === '/auth' ? { mode: 'register' } : undefined}
                className="hbp"
              >
                {currentSlideData.b1}
              </Link>
              <Link 
                to={currentSlideData.b2link} 
                state={currentSlideData.b2link === '/auth' ? { mode: 'register' } : undefined}
                className="hbg"
              >
                {currentSlideData.b2}
              </Link>
      ...
      <div className="fs" data-testid="features-section">
        <div className="cn">
          ...
          <div className="newsletter-cta" data-testid="newsletter-section">
            ...
            <p>{isWestern ? 'Get the latest AI tools, tips, and updates for creatives. No spam, ever.' : 'Get the latest AI tools, tips, and updates for African creatives. No spam, ever.'}</p>
          ...
          <div className="payment-methods" data-testid="payment-methods">
            ...
            <div className="payment-logos">
              {isWestern ? (
                <>
                  <div className="payment-logo paypal" title="PayPal" style={{ background: '#1c1f2a', padding: '4px 8px', borderRadius: '3px', fontSize: '10px', fontWeight: 'bold', color: '#c084fc', border: '1px solid rgba(124, 111, 247, 0.3)' }}>
                    <span>PayPal</span>
                  </div>
