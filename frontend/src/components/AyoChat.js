...
function AyoChat({ currentPage = 'default', isLoggedIn = false, userEmail = null }) {
  ...
  const handleOpen = () => {
    ...
    if (!hasGreeted) {
      ...
      const greeting = isLoggedIn 
        ...
        : isWestern
          ? "Hello! 👋 I'm Ayo, your AI guide to Intermaven. I can help you discover our AI tools, answer questions, or get you started. What would you like to know?"
  ...
  const findResponse = (input) => {
    ...
    const getLocalizedResponse = (key) => {
      let response = AYO_RESPONSES[key];
      if (!response) return AYO_RESPONSES['default'];
      if (!isWestern) return response;
      
      if (key === 'what can intermaven do') {
        return "Intermaven is your AI-powered toolkit for creatives and entrepreneurs! I can help you create professional brand kits, compelling music bios, social media content, sync licensing pitches, and business pitch decks. All powered by AI, designed for global creators. 🌍";
      }
      if (key === 'm-pesa') {
        return "We accept credit card, PayPal, Apple Pay, Google Pay, and regional mobile transfers! Here's how it works:\n\n1️⃣ Choose your plan (Creator USD $4.99 or Pro USD $14.99 equivalent)\n2️⃣ Select your preferred payment method\n3️⃣ Complete the checkout / confirmation\n4️⃣ Credits are added instantly!";
      }
      if (key === 'upgrade') {
        return "Yes, you can upgrade anytime! Just go to Settings → Billing in your dashboard. Choose between:\n\n💜 **Creator Plan** - USD $4.99 for 600 credits\n💎 **Pro Plan** - USD $14.99 for 2,500 credits\n\nPaid credits never expire!";
      }
      if (key === 'add credits') {
        return "To add more credits:\n\n1️⃣ Go to Dashboard → Settings → Billing\n2️⃣ Choose a plan:\n   • Creator: 600 credits for USD $4.99 equivalent\n   • Pro: 2,500 credits for USD $14.99 equivalent\n3️⃣ Pay via Card, PayPal, or regional mobile transfer\n4️⃣ Credits added instantly!\n\nPaid credits never expire.";
      }
      if (key === 'what is intermaven') {
        return "Intermaven is an AI-powered platform built for creatives and entrepreneurs to brand, pitch, and grow their businesses globally. 🌍";
      }
    ...
    for (const [key, response] of Object.entries(AYO_RESPONSES)) {
      if (key !== 'default' && lower.includes(key)) {
        return { text: getLocalizedResponse(key) };
      }
    }
    
    // Check for keywords
    if (lower.includes('credit') || lower.includes('cost')) {
      return { text: getLocalizedResponse('how many credits') };
    }
    if (lower.includes('pay') || lower.includes('mpesa') || lower.includes('m-pesa') || lower.includes('venmo') || lower.includes('cash app') || lower.includes('zelle')) {
      return { text: getLocalizedResponse('m-pesa') };
    }
    if (lower.includes('start') || lower.includes('begin') || lower.includes('new')) {
      return { text: getLocalizedResponse('how do i get started') };
    }
    if (lower.includes('tool') || lower.includes('feature')) {
      return { text: getLocalizedResponse('tell me about the ai tools') };
    }
    if (lower.includes('free')) {
      return { text: getLocalizedResponse('free plan') };
    }
    if (lower.includes('hello') || lower.includes('hi') || lower.includes('hey')) {
      return { text: isWestern ? "Hello! 👋 Great to meet you. I'm Ayo, here to help you navigate Intermaven. What would you like to know about our AI tools?" : "Hello! 👋 Great to meet you. I'm Ayo, here to help you navigate Intermaven. What would you like to know about our AI tools for African creatives?" };
    }
